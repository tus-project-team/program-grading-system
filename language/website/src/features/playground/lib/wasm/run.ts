...
// @ts-expect-error todo: fix later by adding types
import cowsayBuffer from "./tools.wasm?uint8array";

export const run = async (buffer: Uint8Array, name: string) => {
  ...
  console.log(mod);
  const { getAst } = mod;
  console.log(getAst("fn main() -> i32 { 0 }"));
};
